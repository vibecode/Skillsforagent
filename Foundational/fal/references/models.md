...
Common model endpoint IDs organized by category. Each model has its own input schema — use the Platform API to discover exact parameters: `GET https://api.fal.ai.proxy.chorus.com/v1/models?endpoint_id={id}&expand=openapi-3.0`
...
curl "https://api.fal.ai.proxy.chorus.com/v1/models?endpoint_id=fal-ai/flux/dev&expand=openapi-3.0" \
...
```bash
curl "https://api.fal.ai.proxy.chorus.com/v1/models?query=text-to-video&limit=10" \
  -H "Authorization: Key $FAL_API_KEY"
```