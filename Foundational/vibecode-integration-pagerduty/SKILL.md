...
**Auth**: Token auth via `PAGERDUTY_TOKEN`. PagerDuty uses a non-standard header format:
...
curl -s -X PUT -H "Authorization: Token token=$PAGERDUTY_TOKEN" -H "Content-Type: application/json" \
  "https://api.pagerduty.com/incidents/{incident_id}" \
...
# Create incident
curl -s -X POST -H "Authorization: Token token=$PAGERDUTY_TOKEN" -H "Content-Type: application/json" \
...
- **Auth header format**: `Token token=<key>` (not `Bearer`). This is PagerDuty-specific.
