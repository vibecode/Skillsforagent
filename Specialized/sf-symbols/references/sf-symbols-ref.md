...
#### Related Skills
- Use `sf-symbols.md` for decision trees, anti-patterns, troubleshooting, and when to use which effect
...
4. Asset catalog symbols: `Image("custom.symbol.name")`. For symbols loaded from a bundle: `Image("custom.symbol.name", bundle: .module)`
...
// UIKit uses imperative start/stop (isActive: is SwiftUI-only)
if isLoading {
    barButtonItem.addSymbolEffect(.pulse)
} else {
...
**Skills**: sf-symbols.md (symbol selection and animation guidance)
