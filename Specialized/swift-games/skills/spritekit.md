...
| `.resizeFill` | Resizes scene to match view size | Adaptive scenes (no scaling distortion) |
| `.fill` | Scales each axis independently | Almost never  -  distorts aspect ratio |
...
```swift
// Under Swift 6: declare the class without the conformance, then add it via
// an @MainActor isolated extension. Inlining SKPhysicsContactDelegate in the
// class declaration claims a nonisolated conformance from main-actor code and
// triggers a Swift 6 diagnostic.
@MainActor
class GameScene: SKScene {
...
struct GameView: View {
    // @State keeps the scene stable across SwiftUI redraws.
    // Never create the scene inside body — that rebuilds it on every refresh.
